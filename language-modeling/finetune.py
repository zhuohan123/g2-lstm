...
parser.add_argument('--model', type=str, default='LSTM',
                    help='type of recurrent net (RNN_TANH, RNN_RELU, LSTM, GRU, G2LSTM)')
...
parser.add_argument('--gumbel-noise-p', type=float, default=0.0,
                    help='Gmuble_noise_p in Gumbel gate')
parser.add_argument('--gumbel-noise-t', type=float, default=1.0,
                    help='Gmuble_noise_t in Gumbel gate')
parser.add_argument('--gumbel-noise-type', type=str, default='new_U_B',
                    help='Gmubel_noise_type in Gumbel gate (new_U_B, new_U, no_new)')
parser.add_argument('--divide-temp', type=float, default=None,
...
model = model.RNNModel(args.model, ntokens, args.emsize, args.nhid, args.nlayers,
                       ...
                       args.gumbel_noise_p, args.gumbel_noise_t, args.gumbel_noise_type,
                       args.divide_temp,
                       args.tied)
if args.cuda:
    model.cuda()
...
with open(args.load, 'rb') as f:
    model = torch.load(f)
# if args.model == 'G2LSTM':
#     model.reset_gumbel_noise(args.gumbel_noise_p, args.gumbel_noise_t, args.gumbel_noise_type)

# Loop over epochs.
lr = args.lr
stored_loss = evaluate(val_data)
message('| stored loss | valid loss {:5.2f} | valid ppl {:8.2f}'.format(stored_loss, math.exp(stored_loss)))
best_val_loss = []
