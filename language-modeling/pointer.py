...
parser.add_argument('--compression', type=str,  default='',
                    help='compress the model, (svd, precision, clip)')
...
def evaluate(data_source, batch_size=10, window=args.window):
    ...
    for i in range(0, data_source.size(0) - 1, args.bptt):
        ...
        words = data.data.cpu().numpy()[:, 0]
        words = [corpus.dictionary.idx2word[word] for word in words]
        print(words)
...
def compression(x):
    if args.compression == 'clip':
        x.clamp_(-2 * args.compression_k, 2 * args.compression_k)
