...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers,
        ...
        assert rnn_type in ['LSTM', 'QRNN', 'GRU', 'G2LSTM'], 'RNN type is not supported'
        if rnn_type == 'LSTM':
            ...
            self.rnns = [g2_lstm.LSTM(g2_lstm.LSTMCell,
                                      ninp if l == 0 else nhid,
                                      nhid if l != nlayers - 1 else (ninp if tie_weights else nhid),
                                      1,
                                      dropout=0,
                                      wdrop=wdrop,
                                      gumbel_noise_p=gumbel_noise_p,
                                      gumbel_noise_t=gumbel_noise_t,
                                      gumbel_noise_type=gumbel_noise_type,
                                      divide_temp=divide_temp) for l in range(nlayers)]

        message(repr(self.rnns))
    ...
    def reset_gumbel_noise(self, gumbel_noise_p=0.0, gumbel_noise_t=1.0,
        ...
        for l, rnn in enumerate(self.rnns):
            rnn.reset_gumbel_noise(gumbel_noise_p, gumbel_noise_t,
                                   gumbel_noise_type)
